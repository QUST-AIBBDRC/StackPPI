...
shu_scale=scale(shu)
train_shu=mds(shu_scale,n_components=300)
...
def get_first_level(train_x, train_y, test_x,num_class):
    rf_model1=RandomForestClassifier(max_depth=15,n_estimators=500)
    ext_model1=ExtraTreesClassifier(n_estimators=500)
    rf_model2=RandomForestClassifier(max_depth=15,n_estimators=500)
...
yscore_sum.to_csv('M_MDS_yscore.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('M_MDS_ytest.csv')
...
data_csv.to_csv('M_MDS_result.csv')
