...
train_data_1=scale(train_data1)
xgb_model=xgb.XGBClassifier(n_estimators=500,max_depth=15)
xgbresult1=xgb_model.fit(train_data_1,train_label1.ravel())
feature_importance=xgbresult1.feature_importances_
feature_number=-feature_importance
H1=np.argsort(feature_number)
mask=H1[:100]
train_data_=train_data_1[:,mask]
sio.savemat('Matine_xgb_mask_100_4.mat',{'mask':mask})
############################
shu_1=np.column_stack([train_label1,train_data_])
...
yscore_sum.to_csv('M_test_yscore_100_4.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('M_test_ytest_100_4.csv')
...
data_csv.to_csv('M_test_result_100_4.csv')
