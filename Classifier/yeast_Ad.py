...
cv_clf = AdaBoostClassifier(n_estimators=500)
...
for train, test in skf.split(X,y): 
    ...
    print('xgb:acc=%f,precision=%f,npv=%f,sensitivity=%f,specificity=%f,mcc=%f,f1=%f,roc_auc=%f'
...
yscore_sum.to_csv('M_Ad_yscore.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('M_Ad_ytest.csv')
...
data_csv.to_csv('M_Ad_result.csv')
